...
class KitManager
{
    ...
    public function deleteKit(string $name): void
    {
        if (!isset($this->kits[$name])) {
            throw new Exception("ERROR: The kit does not exist");
        }
    ...
    private function loadKits(): void
    {
        ...
        if (!is_array($files) || count($files) === 0) {
            throw new Exception("No JSON files found in directory: {$this->directory}");
        }
    ...
    private function saveKit(Kit $kit): void
    {
    ...
    /* This will be used later in a plugin update...
    public function updateKit(Kit $kit): void
    {
        if ($this->kitExists($kit->getName())) {
            $this->kits[$kit->getName()] = $kit;
            $this->saveKit($kit);
        }
    }
